...
def main_worker(gpu, args):
    ...
    for set_id in test_sets_list:
        ...
        # --- Handle ImageNet-C (Weather/Corruption) ---
        if set_id == 'C':
            # Define corruptions to iterate over
            if args.corruption == 'weather':
                # Standard ImageNet-C weather corruptions
                corruption_list = ['snow', 'frost', 'fog', 'brightness']
            elif args.corruption is None:
                 raise ValueError("For ImageNet-C (set_id='C'), you must provide --corruption (e.g., 'snow' or 'weather').")
            else:
                corruption_list = [args.corruption]

            for corruption_name in corruption_list:
                print(f"evaluating: ImageNet-C / {corruption_name} / severity {args.severity}")
                
                # Reset classnames for ImageNet-C (Uses standard ImageNet classes)
                classnames = imagenet_classes
                ...
                key = f"C_{corruption_name}_{args.severity}"
                results[key] = test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args)
                
                del val_dataset, val_loader
                print("=> Acc. on testset [{}]: @1 {}/ @5 {}".format(key, results[key][0], results[key][1]))

        # --- Handle Standard Datasets (A, R, V, K, I) ---
        else:
            ...
            # Standard loading logic from original script
...
if __name__ == '__main__':
    ...
    # --- New Arguments for ImageNet-C ---
    parser.add_argument('--corruption', type=str, default=None, 
                        help='corruption type for ImageNet-C (e.g., snow, frost, fog, brightness, or "weather" for all)')
