...
def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args):
    ...
    # List per salvare l'andamento dell'accuratezza nel tempo (per i grafici)
    continuous_history = []
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        # LOGICA RESET PROMPT
        # Originale: resettava sempre se non era cocoop.
        # Nuova Logica: Resetta solo se NON siamo in continuous, oppure se siamo in un intervallo di reset.
        
        if not args.cocoop: # no need to reset cocoop because it's fixed
            if args.tta_steps > 0:
                # Determiniamo se dobbiamo resettare
                should_reset = True
                
                if args.continuous:
                    should_reset = False
                    # Se l'utente ha specificato un intervallo (es. ogni 100 immagini)
                ...
                if should_reset:
                    with torch.no_grad():
                        model.reset()
                    # Ricarichiamo anche lo stato iniziale dell'optimizer (momentum azzerato)
                    optimizer.load_state_dict(optim_state)
            
            # Eseguiamo il tuning
            # Se siamo in continuous e non abbiamo resettato, l'optimizer e il model
            # mantengono lo stato dell'iterazione precedente!
        ...
        # CONTINUOUS LOGGING: Salviamo step e accuracy corrente
        if args.continuous:
            continuous_history.append([i, acc1[0].item(), top1.avg.item()])
    ...
    if args.continuous:
        ...
        with open(csv_path, 'w', newline='') as csvfile:
            ...
            writer.writerow(['Step', 'Instant_Acc', 'Avg_Acc'])
